...
---
### 2026.05.11. Chapter 07-1. 
- 인공 신경망의 구조
  - 입력층, 밀집층(완전 연결층), 출력층
  - 모델 파라미터? 모델이 학습을 통해 스스로 찾아내는 가중치와 절편
- 케라스 모델
  - Input : 입력층 생성
  - Dense : 밀집층 생성
  - Sequential : 입력층, 밀집층을 가진 신경망 모델
- 타깃값이 정수일 때 손실 함수
...
---
### 2026.05.07. Chapter 06-3. 주성분 분석
...
---
### 2026.05.05. Chapter 06-2. k-평균
- 사이킷런의 KMeans 클래스
- k-평균 알고리즘 구현 
- 속성
  - labels_ : 각 샘플이 어떤 클러스터에 소속되어 있는지
  - inertia_ : 이너셔. 샘플-클러스터 거리 제곱합
  - cluster_centers : 클러스터 중심점 
  - n_iter_ : 최적의 클러스터 찾기 위해 반복 횟수
- 메서드
  - transform() : 각 샘플에서 각 클러스터까지의 거리 
  - predict() : 가장 가까운 클러스터를 예측값으로 출력
- k값을 미리 지정해야 하는데, 최적의 k값을 알아내기 위해 **이너셔** 사용
  - 이너셔가 더 이상 크게 줄어들지 않는다면 클러스터 개수 늘리는 것은 효과가 없음
  - 이너셔는 각 클러스터가 얼마나 조밀하게 잘 뭉쳐있느냐를 보여줌
  - 클러스터 개수와 이너셔 값의 그래프가 꺾이는 지점. 엘보우
---
